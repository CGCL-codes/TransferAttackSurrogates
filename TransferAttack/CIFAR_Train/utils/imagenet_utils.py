...
def datasetImageNett(root='./data', train=True, transform=None):
    if train: root = os.path.join(root, 'imagenette/train')
    else: root = os.path.join(root, 'imagenette/val')
    return torchvision.datasets.ImageFolder(root=root, transform=transform)

def datasetImageNet10(root='./data', train=True, transform=None):
    if train: root = os.path.join(root, 'imagenet-10-train')
    else: root = os.path.join(root, 'imagenet-10-val')
    return torchvision.datasets.ImageFolder(root=root, transform=transform)


def datasetTinyImageNet(root='./data', train=True, transform=None):
    if train: root = os.path.join(root, 'tiny-imagenet-200/train')
    else: root = os.path.join(root, 'tiny-imagenet-200/val')
    return torchvision.datasets.ImageFolder(root=root, transform=transform)
...
def get_transforms(dataset, train=True):
    assert (dataset == 'imagenet' or dataset == 'imagenett'or dataset == 'imagenet-10' )
    if train:
        comp = [
...
def get_tiny_transforms(dataset, train=True):
    assert (dataset == 'tiny-imagenet' )
    if train:
        comp = [
...
def get_dataset(dataset, root='./data', train=True):
    assert (dataset == 'imagenet' or dataset == 'imagenet-mini' or dataset == 'imagenett' or dataset =='tiny-imagenet' or dataset == 'imagenet-10')
    if dataset == 'imagenet' or dataset == 'imagenet-mini' or dataset == 'imagenett'  or dataset == 'imagenet-10':
...
def get_poisoned_loader(
        ...
        noise_path=None, noise_rate=1.0,  fitr=None):
